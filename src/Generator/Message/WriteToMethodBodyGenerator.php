...
class WriteToMethodBodyGenerator extends BaseGenerator
{
    ...
    public function generateFieldWriteStatement(FieldDescriptorProto $field)
    {
        ...
        if ($type === Type::TYPE_ENUM()) {
            ...
            $body[] = $this->generateWriteScalarStatement($type->value(), '$this->' . $name . '->value()') . ';';
        ...
        if ($type !== Type::TYPE_MESSAGE()) {
            $body[] = sprintf('$writer->writeVarint($stream, %s);', $key);
            $body[] = $this->generateWriteScalarStatement($type->value(), '$this->' . $name) . ';';

            return $body;
        }

        $body[] = '$writer->writeVarint($stream, ' . $key . ');';
        $body[] = '$writer->writeVarint($stream, $this->' . $name . '->serializedSize($sizeContext));';
        $body[] = '$this->' . $name . '->writeTo($context);';
